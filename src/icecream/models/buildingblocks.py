...
class SingleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order='gcr', num_groups=8,
                 padding=1, dropout_prob=0.1, is3d=True, use_bias = True):
        super(SingleConv, self).__init__()

        for name, module in create_conv(in_channels, out_channels, kernel_size, order,
                                        num_groups, padding, dropout_prob, is3d, use_bias):
...
class DoubleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, encoder, kernel_size=3, order='gcr',
                 num_groups=8, padding=1, upscale=2, dropout_prob=0.1, is3d=True,use_bias = True):
        ...
        self.add_module('SingleConv1',
                        SingleConv(conv1_in_channels, conv1_out_channels, kernel_size, order, num_groups,
                                   padding=padding, dropout_prob=dropout_prob1, is3d=is3d, use_bias=use_bias))
        # conv2
        self.add_module('SingleConv2',
                        SingleConv(conv2_in_channels, conv2_out_channels, kernel_size, order, num_groups,
                                   padding=padding, dropout_prob=dropout_prob2, is3d=is3d, use_bias=use_bias))
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 ...
                 num_groups=8, padding=1, upscale=2, dropout_prob=0.1, is3d=True,positional_encoding=False,use_bias= True):
...
def create_encoders(in_channels, f_maps, basic_module, conv_kernel_size, conv_padding,
                    ...
                    layer_order, num_groups, pool_kernel_size, is3d, positional_encoding,pool_type  = 'max'):
