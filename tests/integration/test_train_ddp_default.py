"""
Integration tests for `icecream train` command using DDP on multiple GPUs with defaults setting follows the test 
structure of test_train_single_default.py.
"""
...
# skip tests if less than 2 GPUs are available
import torch
import pytest


if not torch.cuda.is_available() or torch.cuda.device_count() < 2:
    pytest.skip("multi-GPU integration tests require >= 2 GPUs", allow_module_level=True)
...
def test_default_training(tmp_path: Path):
    """
    Integration test for `icecream train` with default setting using DDP on multiple GPUs.
    """
